class Person < ActiveRecord::Base
  ...
  has_many :names, :class_name => 'PersonName', :foreign_key => :person_id, :dependent => :destroy, :conditions => 'person_name.voided = 0', :order => 'person_name.preferred DESC, person_name.given_name ASC'
  ...
  has_many :personattributes, :class_name => "PersonAttribute", :foreign_key => :person_id, :dependent => :destroy
  ...
  def surgeon_name
    "#{self.names.first.prefix.strip} #{self.names.first.given_name.strip} #{self.names.first.family_name.strip} (#{self.user.system_id})".strip rescue nil
  end
  ...
  def name
    "#{self.names.first.family_name}, #{self.names.first.given_name}" rescue nil
  end  
