...
class User < ActiveRecord::Base
 ...
 #validates_length_of:password, :within => 4..50
  ...
  has_many :alerts, :through => :notification_alert_recipient, :foreign_key => :user_id
...
#user_id
  ...
  def name
    "#{self.person.names.first.given_name} #{self.person.names.first.family_name}" rescue nil
  end
  
  def first_name
    "#{self.person.names.first.given_name}" rescue nil
  end
  
  def middle_name
    "#{self.person.names.first.middle_name}" rescue nil
  end
  
  def last_name
    "#{self.person.names.first.family_name}" rescue nil
  end
  
  def isProvider
    self.roles.each{|r|
        if r.role.upcase == "PROVIDER"
            return true
        end
    }
    return false
  ...
  def isSurgicalSecretary
    self.roles.each{|r|
        if r.role.upcase == "SURGICAL SECRETARY"
            return true
        end
    }
    return false
  ...
  def isAnaestheticalSecretary
    self.roles.each{|r|
        if r.role.upcase == "ANAESTHETICAL SECRETARY"
            return true
        end
    }
    return false
